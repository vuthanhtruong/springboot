...
public class Blogs {
    ...
    private Persons creator; // Liên kết với bảng Person (có thể là Student, Teacher, Employee)

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "EventID", nullable = true, foreignKey = @ForeignKey(name = "FK_Blogs_Event"))
    private Notifications event; // Liên kết với bảng Events (có thể null nếu không liên quan đến sự kiện nào)
